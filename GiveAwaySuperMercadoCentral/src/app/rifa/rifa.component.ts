...
export class RifaComponent implements OnInit {
  ...
  sorteo(item: any) {
    ...
    //   obj = this.chosenElements[i];
    //   index = this.chosenElements.findIndex((o: any) => o.Cedula === obj.Cedula);
    //   while (index !== i) {
    //     this.chosenElements[index] = this.excelFinal[Math.floor(Math.random() * this.excelFinal.length)];
    //     index = this.chosenElements.findIndex((o: any) => o.Cedula === obj.Cedula);
    //   }
    // }
    ...
    setTimeout(() => {
      ...
      function mostrarAlerta(i: any) {
        if (i < self.chosenElements.length) {
          ...
          Swal.fire({
            title: '<h1>' + '<b>SUPERMERCADO CENTRAL TE PREMIA!!</b>' + '</h1>' +'<br>'+ '<h1>' + '<b>GANADOR  #</b>' + (i + 1) + '</h1>' +'<br>',
            width: 1200,
            padding: '7em',
            html: '<h2>' + '<b>Nombre:</b>' + '</h2>' + '<h2>' + self.chosenElements[i].Nombre + '</h2>' + '<br>' + '<h2>'+'<b>Boleta:</b>'+'</h2>' + '<h2>' + self.chosenElements[i].Boleta + '</h2>' + '<h2>' + '<br>' + '<h2>' + '<b>Cedula:</b>' + '</h2>' + '<h2>' + self.chosenElements[i].Cedula.substr(0, 3) + "******" + self.chosenElements[i].Cedula.substr(self.chosenElements[i].Cedula.length - 3) + '</h2>' + '<br>' + '<h2>' + '<b>Factura:</b>' + '</h2>' + '<h2>' + self.chosenElements[i].Factura + '</h2>' + '<br>' + '<h2>' + '<b>Teléfono:</b>' + '</h2>' + '<h2>' + self.chosenElements[i].Telefono.substr(0, 3) + "******" + self.chosenElements[i].Telefono.substr(self.chosenElements[i].Telefono.length - 3) + '</h2>',
            color: '#218e27',
